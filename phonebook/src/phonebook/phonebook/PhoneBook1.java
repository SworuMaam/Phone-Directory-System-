...
public class PhoneBook1 implements Serializable {

	private PhoneBook2 head;
	private int length;
	...
	public void add(String name, String address, String phoneNumber) {
		...
		PhoneBook2 current = head;
		PhoneBook2 previous = null;
		PhoneBook2 newNode = new PhoneBook2();
		// Set the data
	...
	// create method to search through list for addresses
	public void addressSearch(String email) {
		PhoneBook2 current = head;
		boolean empty = true;
		...
		if (isEmpty()) {
			...
			for (int i = 0; i < length; i++) {
				if (current.getAddress().toLowerCase().contains(email.toLowerCase())) {
					System.out.print("Index = " + (i+1) + " ");
					System.out.println(current);
					empty = false;
				}
			...
			if (empty){
				System.out.println("No matches to that address!");
			}
		}
